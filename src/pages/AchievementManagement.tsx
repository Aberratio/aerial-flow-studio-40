...
const AchievementManagement = () => {
  ...
  const fetchAchievements = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('achievements').select('*').order('created_at', {
        ascending: false
      });
  ...
  const ruleTypes = [{
    ...
    value: 'figures_completed',
    label: 'Figures Completed'
  }, {
    value: 'post_likes',
  ...
  return <div className="min-h-screen p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-bold text-white my-[32px]">Achievement Management</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {achievements.map(achievement => <Card key={achievement.id} className="glass-effect border-white/10">
        ...
        {achievements.length === 0 && <Card className="glass-effect border-white/10">
        ...
        <DialogContent className="max-w-md bg-black/95 border-white/10">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              ...formData,
              name: e.target.value
            })} placeholder="Achievement name" required className="bg-white/5 border-white/10 text-white" />
            </div>
            ...
            })} placeholder="Achievement description" required className="bg-white/5 border-white/10 text-white" />
            </div>

            <div>
              <label className="block text-sm font-medium text-white mb-2">Icon (Emoji)</label>
              <Input value={formData.icon} onChange={e => setFormData({
              ...formData,
              icon: e.target.value
            })} placeholder="🏆" required className="bg-white/5 border-white/10 text-white" />
            </div>
            ...
            })} placeholder="100" required min="0" className="bg-white/5 border-white/10 text-white" />
            </div>

            <div>
              <label className="block text-sm font-medium text-white mb-2">Rule Type</label>
              <Select value={formData.rule_type} onValueChange={value => setFormData({
              ...formData,
              rule_type: value
            })}>
                ...
                <SelectContent className="bg-black/95 border-white/10">
                  {ruleTypes.map(type => <SelectItem key={type.value} value={type.value} className="text-white hover:bg-white/10">
                      {type.label}
                    </SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div>
              ...
              <Input type="number" value={formData.rule_value} onChange={e => setFormData({
              ...formData,
              rule_value: Number(e.target.value)
            })} placeholder="1" required min="1" className="bg-white/5 border-white/10 text-white" />
            </div>

            <div className="flex space-x-3 pt-4">
              <Button type="button" variant="outline" onClick={() => setIsModalOpen(false)} className="flex-1 border-white/20 text-white hover:bg-white/10">
