...
const SkillTree = ({ sportCategory, sportName, onBack }: SkillTreeProps) => {
  ...
  const [userChallengeParticipations, setUserChallengeParticipations] = useState<{[challengeId: string]: {participating: boolean, completed: boolean}}>({});
    ...
    try {
      ...
      for (const level of levelsData) {
        ...
        if (calculatedPoints >= level.point_limit) {
          ...
          if (level.challenge_id && userChallengeParticipations[level.challenge_id]?.completed) {
  ...
  const fetchUserChallengeParticipations = async () => {
    ...
    try {
      ...
      const participations: {[challengeId: string]: {participating: boolean, completed: boolean}} = {};
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center flex-wrap gap-4 mb-4">
            ...
            <p className="text-muted-foreground">
              Complete figures to earn 1 point × level number, challenges for 3 points × level number
            </p>
          </div>
            ...
            sportLevels.map((level) => {
              ...
              return (
                <Card key={level.id} className={`glass-effect border-white/10 transition-all duration-300 ${
                        ...
                        <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-400/20 rounded-lg p-3 md:p-4 mt-4">
                          <div className="flex flex-col md:flex-row md:items-start gap-3 md:gap-4">
                            ...
                            <div className="flex flex-row md:flex-col gap-2 w-full md:w-auto">
                              {isUnlocked ? (
                                userChallengeParticipations[level.challenges!.id]?.participating ? (
                                  <div className="flex flex-col md:flex-col gap-2 w-full">
                                    <Button
                                      onClick={() => navigate(`/challenges/${level.challenges!.id}`)}
                                      variant="outline"
                                      className={`${
                                        userChallengeParticipations[level.challenges!.id]?.completed
                  ...
                  {(showAllLevels || isUnlocked || (level.challenges && userChallengeParticipations[level.challenges.id]?.participating && !userChallengeParticipations[level.challenges.id]?.completed)) && (
                    <CardContent>
