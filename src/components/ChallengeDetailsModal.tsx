...
export const ChallengeDetailsModal = ({
  ...
  const progressPercentage =
    (challenge.completedDays / challenge.totalDays) * 100;
  ...
  const handleContinue = () => {
    onClose();
    // Use onContinue prop if provided, otherwise fallback to local logic
    if (onContinue) {
      onContinue();
      return;
    }

    // Fallback: navigate to challenge preview page
    navigate(`/challenges/${challenge.id}`);
  };

  const handleStart = () => {
    onClose();
    // Use onStart prop if provided, otherwise fallback to local logic
    if (onStart) {
      onStart();
      return;
    }
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-white font-medium flex items-center">
                      ...
                      {day.day === challenge.currentDay && !day.completed && (
