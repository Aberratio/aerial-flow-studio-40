  ...
  const fetchFigurePosts = async () => {
    ...
    try {
      ...
      if (postsData) {
        ...
        const postsWithMedia = postsWithCounts.filter(
          (post) => post.image_url || post.video_url
        );
        const community = postsWithMedia.filter(
          (post) =>
            post.user_id !== user.id && !friendIdList.includes(post.user_id)
        );
        const friends = postsWithMedia.filter((post) =>
      ...
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto glass-effect border-white/10 p-0">
          ...
          <DialogDescription className="sr-only">
            {figure?.description ||
              `Details and progress tracking for ${figure?.name}`}
          </DialogDescription>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      {figureProgress ? (
                        <Badge
                          className={getStatusColor(figureProgress.status)}
                        >
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-4">
                      ...
                      <Button
                        ...
                        className={`border-gray-500/30 text-gray-400 hover:bg-gray-500/20 ${
                          figureProgress?.status === "not_tried" ||
                          !figureProgress
                            ? "bg-gray-500/20"
                            : ""
                        }`}
                                ...
                                <Avatar
                                  className={`w-10 h-10 ${
                                    post.user.role === "trainer"
                                      ? "ring-2 ring-gradient-to-r from-yellow-400 to-orange-500 ring-offset-2 ring-offset-black"
                                      : ""
                                  }`}
                                >
                                  <AvatarImage
