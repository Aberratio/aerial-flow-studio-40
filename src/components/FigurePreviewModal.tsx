...
export const FigurePreviewModal = ({
  figure,
  isOpen,
  onClose
  ...
  const fetchFigurePosts = async () => {
    ...
    try {
      ...
      const {
        ...
        `).eq('figure_id', figure.id).order('created_at', {
        ascending: false
      });
      ...
      if (postsData) {
        ...
        const postsWithCounts = await Promise.all(postsData.map(async post => {
          ...
          const {
            data: userLike
          } = await supabase.from('post_likes').select('id').eq('post_id', post.id).eq('user_id', user.id).maybeSingle();
          return {
            id: post.id,
  ...
  const fetchFigureProgress = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from('figure_progress').select('*').eq('user_id', user.id).eq('figure_id', figure.id).maybeSingle();
  ...
  const updateFigureStatus = async (status: string) => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('figure_progress').upsert({
        user_id: user.id,
        figure_id: figure.id,
        status: status,
  ...
  return <>
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto glass-effect border-white/10 p-0">
          ...
          <div className="flex flex-col lg:flex-row min-h-[80vh]">
            ...
            <div className="lg:w-1/2 bg-black relative">
              ...
              {figure.video_url ? <div className="relative w-full h-full min-h-[400px] flex items-center justify-center">
                  <video src={figure.video_url} className="w-full h-full object-contain" controls autoPlay={false} />
                </div> : figure.image_url ? <div className="relative w-full h-full min-h-[400px] flex items-center justify-center">
                  <img src={figure.image_url} alt={figure.name} className="w-full h-full object-contain" />
                </div> : <div className="w-full h-full min-h-[400px] flex items-center justify-center text-muted-foreground">
            ...
            <div className="lg:w-1/2 flex flex-col bg-black/50 backdrop-blur-sm">
              ...
              <div className="p-6 border-b border-white/10">
                <div className="flex items-start justify-between mb-4 my-[23px]">
                  ...
                  <div className="flex items-center space-x-2 my-0">
                    ...
                    <Button onClick={() => setShowShareModal(true)} size="sm" variant="ghost" className="text-muted-foreground hover:text-white">
                ...
                {figure.description && <p className="text-muted-foreground mb-4">{figure.description}</p>}

                <div className="flex flex-wrap gap-2 mb-4">
                  {figure.category && <Badge variant="outline" className="border-white/20 text-white/80">
                      {figure.category}
                    </Badge>}
                  {figure.type && <Badge variant="outline" className="border-white/20 text-white/80">
                      {figure.type}
                    </Badge>}
                  {figure.tags && figure.tags.map((tag, index) => <Badge key={index} variant="outline" className="border-purple-500/30 text-purple-300">
                 ...
                 <div className="space-y-3">
                   ...
                   <div className="flex items-center justify-between">
                     <div className="flex items-center space-x-2">
                       ...
                       {figureProgress ? <Badge className={getStatusColor(figureProgress.status)}>
                           {getStatusIcon(figureProgress.status)}
                           <span className="ml-1 capitalize">{figureProgress.status.replace('_', ' ')}</span>
                         </Badge> : <Badge className={getStatusColor('not_tried')}>
                           {getStatusIcon('not_tried')}
                           <span className="ml-1">Not Tried</span>
                         </Badge>}
                     </div>
                     <Button variant="ghost" size="sm" onClick={() => setShowStatusOptions(!showStatusOptions)} className="text-muted-foreground hover:text-white">
                       <Edit className="w-4 h-4" />
                     </Button>
                   </div>
                   
                   {/* Status Options - Only show when editing */}
                   {showStatusOptions && <div className="grid grid-cols-2 gap-2 mt-4">
                       <Button variant="outline" size="sm" onClick={() => updateFigureStatus('completed')} disabled={updatingStatus} className={`border-green-500/30 text-green-400 hover:bg-green-500/20 ${figureProgress?.status === 'completed' ? 'bg-green-500/20' : ''}`}>
                         <CheckCircle className="w-4 h-4 mr-2" />
                         Completed
                       </Button>
                       <Button variant="outline" size="sm" onClick={() => updateFigureStatus('for_later')} disabled={updatingStatus} className={`border-blue-500/30 text-blue-400 hover:bg-blue-500/20 ${figureProgress?.status === 'for_later' ? 'bg-blue-500/20' : ''}`}>
                         <Bookmark className="w-4 h-4 mr-2" />
                         For Later
                       </Button>
                       <Button variant="outline" size="sm" onClick={() => updateFigureStatus('failed')} disabled={updatingStatus} className={`border-red-500/30 text-red-400 hover:bg-red-500/20 ${figureProgress?.status === 'failed' ? 'bg-red-500/20' : ''}`}>
                         <AlertCircle className="w-4 h-4 mr-2" />
                         Failed
                       </Button>
                       <Button variant="outline" size="sm" onClick={() => updateFigureStatus('not_tried')} disabled={updatingStatus} className={`border-gray-500/30 text-gray-400 hover:bg-gray-500/20 ${figureProgress?.status === 'not_tried' || !figureProgress ? 'bg-gray-500/20' : ''}`}>
              ...
              {/* Instructions */}
              {figure.instructions && <div className="p-6 border-b border-white/10">
              ...
              <div className="flex-1 p-6">
                ...
                <Tabs defaultValue="community" className="w-full">
                  ...
                  <TabsContent value="community" className="mt-4">
                    {loading ? <div className="text-center py-8 text-muted-foreground">Loading...</div> : communityPosts.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                        No community posts yet for this figure
                      </div> : <div className="space-y-3 max-h-[400px] overflow-y-auto">
                        {communityPosts.map(post => <Card key={post.id} className="glass-effect border-white/10 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => {
                  ...
                  <TabsContent value="friends" className="mt-4">
                    {loading ? <div className="text-center py-8 text-muted-foreground">Loading...</div> : friendsPosts.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                        Your friends haven't posted about this figure yet
                      </div> : <div className="space-y-3 max-h-[400px] overflow-y-auto">
                        {friendsPosts.map(post => <Card key={post.id} className="glass-effect border-white/10 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => {
                      setSelectedPost(post);
                      setIsPostModalOpen(true);
                    }}>
                            <CardContent className="p-4">
                  ...
                  <TabsContent value="mine" className="mt-4">
                    {loading ? <div className="text-center py-8 text-muted-foreground">Loading...</div> : myPosts.length === 0 ? <div className="text-center py-8 text-muted-foreground">
                        You haven't posted about this figure yet
                      </div> : <div className="space-y-3 max-h-[400px] overflow-y-auto">
                        {myPosts.map(post => <Card key={post.id} className="glass-effect border-white/10 cursor-pointer hover:bg-white/5 transition-colors" onClick={() => {
                      setSelectedPost(post);
                      setIsPostModalOpen(true);
                    }}>
                            <CardContent className="p-4">
      ...
      {/* Post Preview Modal */}
      <PostPreviewModal post={selectedPost} isOpen={isPostModalOpen} onClose={() => {
      setIsPostModalOpen(false);
      setSelectedPost(null);
    }} />

      {/* Create Post Modal */}
      <CreatePostModal isOpen={showCreatePost} onClose={() => setShowCreatePost(false)} onPostCreated={newPost => {
      ...
      <ShareExerciseModal isOpen={showShareModal} onClose={() => setShowShareModal(false)} exerciseId={figure?.id || ''} exerciseName={figure?.name || ''} />
    </>;
};