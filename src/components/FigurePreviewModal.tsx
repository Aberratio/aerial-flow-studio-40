...
import { X, Play, Clock, Target, User, Tag } from 'lucide-react';
  ...
  const fetchFigurePosts = async () => {
    ...
    try {
      ...
      if (postsData) {
        ...
        // Separate community posts and user's posts
        const community = postsWithCounts.filter(post => post.user_id !== user.id);
        ...
        <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto glass-effect border-white/10 p-0">
          <div className="flex flex-col lg:flex-row min-h-[80vh]">
            ...
            <div className="lg:w-1/2 bg-black relative">
              <Button
                onClick={onClose}
                className="absolute top-4 right-4 z-10 w-8 h-8 p-0 bg-black/50 hover:bg-black/70"
                variant="ghost"
              >
