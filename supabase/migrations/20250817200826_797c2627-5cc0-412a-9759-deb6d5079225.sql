...
SELECT 
  ...
  'security_update',
  jsonb_build_object(
    'update_type', 'rls_privacy_fix',
    'timestamp', now(),
    'description', 'Implemented privacy-aware RLS policies for posts and related tables'
  )
WHERE EXISTS (SELECT 1 FROM public.user_activities LIMIT 1);
